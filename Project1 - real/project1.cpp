...
int Game::showTopPlayers(string text3_param, string diff3){
    ...
    while(getline(showStatsFile,current_line)){
        ...
        if(found_string>18 && diff3 == "Elementary"){
            ...
            if(current_line.find("User: computer&")){
                cout<< "foundcomp"<<endl;
                ...
                cout<< "found"<<endl;
                cout<< current_line.find("User: ")<< endl;
                players[get_current_player] = {current_line.substr(current_line.find("User: ") +6), stoi(current_line.substr(current_line.find("Victories: ") +11)),
                                                stoi(current_line.substr(current_line.find("Defeats: ") +9)),
                                                stoi(current_line.substr(current_line.find("Ties: ") +6))
                                            };
            ...
            if(current_line.find("User: computer")){
                ...
                player[get_current_player] = {current_line.substr(current_line.find("User: ") +6), stoi(current_line.substr(current_line.find("Victories: ") +11)),
                                                stoi(current_line.substr(current_line.find("Defeats: ") +9)),
                                                stoi(current_line.substr(current_line.find("Ties: ") +6))
                                            };
...
int Game::loadStats(string text2_param, string user2, string diff2){
    ...
    // string current_line="";
    // // size_t symbol_find, symbol_find_2;
    ...
    //     // symbol_find=current_line.find("User: ");
    //     // line_object=current_line.substr(symbol_find+6, symbol_find+user2.length());
    //     // cout<<line_object<<endl;
    //     // // cout<<symbol_find<<endl;
    //     // symbol_find_2 = current_line.find("Difficulty: ");
    //     // line_object_2 =current_line.substr(symbol_find_2+12, symbol_find+diff2.length());
    //     // cout<<line_object_2<<endl;
    //     size_t symbol_find = current_line.find("User: ");
    //     size_t symbol_find_2 = current_line.find(" Difficulty: ");

    //     if (symbol_find != string::npos && symbol_find_2 != string::npos) {
    //         line_object = current_line.substr(symbol_find+6, user2.length());
    //         cout<<line_object<<endl;
    //         line_object_2 = current_line.substr(symbol_find_2+13, diff2.length());
    //         cout <<line_object_2<<endl;
    //     }

    //     if((line_object.compare(user2)) && (line_object_2.compare(diff2))){
    ...
    // }
    // // retrieveFile >>User>> difficulty>>human_defeats>>human_defeats>>human_ties;
    // // retrieveFile >> User >> human >>computer >>inputed_character >> nop >> location_on_game_array >> array_size >>difficulty;
    ...
    while (getline(retrieveFile, current_line)) {
        ...
        if (user_pos != string::npos && diff_pos != string::npos) {
            ...
            if ((line_object==User) && (line_object_2==difficulty)) {
...
int Game::saveGame(){
    ...
    //     myfile <<"User: " << User<< " " << "human:  " << human<< " " <<  "computer:  "<<computer << " " << "nop: "<< nop << " " <<  "location_on_game_array: "<<location_on_game_array<< " " << "array_size: "<<array_size<< " " << "difficulty: "<<difficulty << " ";
    //     myfile << "arrays: ";
    //     for (int i = 0; i < 9; ++i) {
    //         myfile << game_array[i];
    //     }
